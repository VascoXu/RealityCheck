...
var phrasecount = 0;
var totalcount = 0;
var phraselimit = localStorage.getItem('phrases') || 5;
var Allphrases = [];
var allphrases = [];
var PresentString = "";
...
if ($('.ui.sidebar').length > 0) {
    $('.ui.sidebar').sidebar().sidebar('attach events', '.ui.launch.button');
}

// return participant information
function getParticipantInfo() {
    return {
        participantId: localStorage.getItem('participant_id') || 'NA',
        // sessionId: localStorage.getItem('session_id') || 'NA',
        headsetType: localStorage.getItem('headset_type') || 'NA',
        keyboardType: localStorage.getItem('keyboard_type') || 'NA',
...
$("#Enter").click(function() {
    localStorage.setItem('participant_id', $('input[name="participant_id"]').val());
    // localStorage.setItem('session_id', $('input[name="session_id"]').val() );
    localStorage.setItem('keyboard_type', $('select[name="keyboard_type"]').val());
    localStorage.setItem('headset_type', $('select[name="headset_type"]').val());
    localStorage.setItem('phrases', Number($("#Phrases").val()));
    localStorage.setItem('uid', Math.floor(Date.now() / 1000));
...
function handleDownload() {
    ...
    var fname = participantInfo['participantId'] + "_" + participantInfo['uid'] + "_" + participantInfo['headsetType'] + "_" + participantInfo['keyboardType'];
    var dataToDownload = {
        filename: fname, 
        json_data: AllJson,
        csv_data: JsonToCSV(AllJson), 
        participant: participantInfo
    };

    var content = JSON.stringify(dataToDownload, null, '\t')
    $.ajax({
        url: '/download',
        method: 'POST',
    ...
    // download(fname + ".csv", JsonToCSV(AllJson))
    // download(fname + ".json", JSON.stringify(dataToDownload, null, '\t'));
    
    // // Create a zip file
    // var zip = new JSZip();
    // zip.file(fname + ".json", JSON.stringify(dataToDownload, null, '\t'));
    // zip.file(fname + ".csv", JsonToCSV(AllJson));
...
function saveAs(blob, filename) {
    ...
    element.href = "data:application/zip;base64," + blob;
